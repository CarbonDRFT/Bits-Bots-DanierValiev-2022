...
import { fetchProductData, fetchProductInfo } from '../functions/fetchFunctions';
import { addProductToCart, toggleProductInFavourites } from '../redux/slices/shoppingSlice';
...
const ProductInfoPage = (): JSX.Element => {
  ...
  const handleAddToFavourites = () => {
    ...
    if (productInfo) {
      if (!isLiked) {
        toast.success(`Successfully added ${productInfo.title} to the favourites.`)
      } else {
        toast.success(`Successfully removed ${productInfo.title} from the favourites.`);
      }
    }
  }
  ...
  useEffect(() => {
    setSimilarItems(productData.filter(
      product =>
      ...
      <ContentWrapper maxWidth="1280px">
        {!isProductLoading && productInfo ? (
          <div className="two-columns">
            <img src={productInfo.image} alt={productInfo.title} className="two-columns__image" />
            <div className="two-columns__content">
              <h2 className="two-columns__title">{productInfo.title}</h2>
                <p className="two-columns__description">{productInfo.description}</p>
                <p className="two-columns__price"><b>{productInfo.price} kr.</b></p>
            ...
            <div className="similar-items">
              ...
              >
                {productData.length > 0 ? (
                  similarItems.map(
                    item => <ProductBox key={item.id} product={item} isLiked={favourites.includes(item.id)} />
                  )
                ) : null}
              </Collection>
            </div>
